...
class HelpCog(commands.Cog, name="help command"):
	...
	async def help(self, ctx, commandName:str=None):
		...
		helpUtility = ""
		for command in utility:
			helpUtility+=f"``{command}`` "
		...
		if commandName is not None:
			...
			embed.set_thumbnail(url=f'{self.bot.user.avatar_url}')
			embed.add_field(name=f"UTILITY COMMANDS :", value=helpUtility, inline=False)
			embed.add_field(name=f"FUN COMMANDS :", value=helpFun, inline=False)
			embed.add_field(name=f"ACTION COMMANDS :", value=helpAction, inline=False)			
			await ctx.channel.send(embed=embed) 
